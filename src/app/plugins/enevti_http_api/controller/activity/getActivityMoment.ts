...
import { invokeGetActivityMoment } from '../../utils/invoker/redeemable_nft_module';
...
export default (channel: BaseChannel) => async (req: Request, res: Response) => {
  try {
    ...
    const activityMoment = await invokeGetActivityMoment(channel, id);
    const { v, o, c } = createPagination(activityMoment.items.length, version, offset, limit);

    const response = {
      checkpoint: c,
      version: v,
      data: activityMoment.items.slice(o, c).map(item => ({
        date: item.date,
        name: item.name,
